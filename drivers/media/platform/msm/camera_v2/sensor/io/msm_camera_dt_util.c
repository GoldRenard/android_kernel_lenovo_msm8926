...
static int gpio22_req_times = 0;
static int gpio23_req_times = 0;
/* +end */
...
if (of_property_read_bool(of_node, "qcom,gpio-vana") == true) {
		...
		CDBG("%s qcom,SENSOR_GPIO_VANA %d\n", __func__,
			gconf->gpio_num_info->gpio_num[SENSOR_GPIO_VANA]);
	}
	...
	for (index = 0; index < ctrl->power_setting_size; index++) {
		...
		case SENSOR_GPIO:
			...
			if (!ctrl->gpio_conf->gpio_num_info->valid
				...
				continue;
/* +add ljk for gpio 22/23 share */
			if(22 == ctrl->gpio_conf->gpio_num_info->gpio_num[power_setting->seq_val])
			{
			    gpio22_req_times++;
			}
			if(23 == ctrl->gpio_conf->gpio_num_info->gpio_num[power_setting->seq_val])
            {
			    gpio23_req_times++;
			}
			CDBG("%s:%d gpio set val %d gpio22_req_times=%d gpio23_req_times=%d\n", __func__, __LINE__,
				ctrl->gpio_conf->gpio_num_info->gpio_num
				[power_setting->seq_val],gpio22_req_times,gpio23_req_times);
/* +end */

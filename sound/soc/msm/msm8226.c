...
#define EXT_SPK_PA_DELAY_DELTA 2
#define EXT_SPK_PA_LATCH_DELTA 500
#define EXT_SPK_PA_TIME_OFF_DELTA 500
...
static unsigned int pa_mode_preset = 0;
...
{
	if (enable) {
        printk("%s set pa mode %d\n",__func__,pa_mode_preset);
        // mode 1
        if(pa_mode_preset == 0){
             gpio_set_value(ext_spk_amp_gpio, 1);
            /* time takes enable the external power amplifier */
            usleep_range(EXT_SPK_PA_LATCH_DELTA,EXT_SPK_PA_LATCH_DELTA);
        }
        //mode 2
        else if(pa_mode_preset == 1){
            gpio_set_value(ext_spk_amp_gpio, 1);
            udelay(EXT_SPK_PA_DELAY_DELTA);
...
static int pa_mode_preset_get_control_value(struct snd_kcontrol *kcontrol,
                                  struct snd_ctl_elem_value *ucontrol)
{
       ucontrol->value.integer.value[0] = pa_mode_preset;

       return 0;
}

static int pa_mode_preset_put_control_value(struct snd_kcontrol *kcontrol,
                                 struct snd_ctl_elem_value *ucontrol)
{
       pa_mode_preset = ucontrol->value.integer.value[0];

       pr_debug("%s: pa_mode_preset=%u\n", __func__, pa_mode_preset);
...
static const struct snd_kcontrol_new msm_snd_controls[] = {
	...
	SOC_ENUM_EXT("SLIM_0_RX SampleRate", msm_snd_enum[4],
        ...
        SOC_SINGLE_EXT("PA Mode Preset",SND_SOC_NOPM, 0, 3, 0,
            pa_mode_preset_get_control_value, pa_mode_preset_put_control_value),
